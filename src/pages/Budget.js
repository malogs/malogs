...
function Budget() {
  ...
  function addExpense(id, name, value, expId) {
    ...
    setIncomes((prev) => {
      ...
      const totExpenses = foundIncome
        ? foundIncome.expenses
            .map((exp) => exp.value)
            .reduce((a, b) => parseFloat(a) + parseFloat(b), 0)
        : 0;
      if (parseFloat(value) + totExpenses > parseFloat(foundIncome.value)) {
        continueProcess = window.confirm(
          "The entered amount makes expenses higher than income! Do you wish to proceed anyways?"
        );
      }
      ...
      if (continueProcess) {
        ...
        return prev.map((incom) => {
          if (incom.id === id)
            return { ...incom, expenses: [...incom.expenses, expense] };
          else return incom;
        });
      } else return prev;
    });
  ...
  function completeExpense(incomeId, expId) {
    if (expId && incomeId) {
      const sol = window.confirm(
        "Are you sure you want to change expense completion?"
      );
      if (sol)
        setIncomes((prev) =>
          prev.map((incom) => {
  ...
  return (
    <div className="container">
      <div className="incomecard__summary">
        ...
        <div>
          {isDirty && (
            <button style={{ marginLeft: "6px" }} onClick={persistData}>
              Save
            </button>
          )}
        </div>
      ...
      <form onSubmit={addIncome}>
        ...
        <input
          ...
          value={incomeValue}
          onChange={(e) => setIncomeValue(e.target.value.trim())}
        />
        <input
          type="date"
        ...
        <ul>
          <li>
            ...
            <span className="value" style={{ color: "green" }}>
              {incomes
                .map((inc) => parseFloat(inc.value))
                .reduce((a, b) => a + b, 0)
                .toLocaleString("en-US", { style: "currency", currency })}
            </span>
          </li>
          <li>
            <b>Total Expenses (Planned): </b>
            <span className="value" style={{ color: "crimson" }}>
              {incomes
                .map((inc) =>
                  parseFloat(
                    inc.expenses
                      .map((exp) => parseFloat(exp.value))
                      .reduce((a, b) => a + b, 0)
                  )
                )
                .reduce((a, b) => a + b, 0)
          ...
          <li>
            <b>
              Total Income (Remaining until{" "}
              {dayjs(lastDate).format("MMMM YYYY")}):{" "}
            </b>
            <span className="value" style={{ color: "green" }}>
              {incomes
                .map(
                  (inc) =>
                    parseFloat(inc.value) -
                    parseFloat(
                      inc.expenses
                        .map((exp) => parseFloat(exp.value))
                        .reduce((a, b) => a + b, 0)
